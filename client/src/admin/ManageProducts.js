...
const ManageProducts = () => {
    ...
    var columns = [
       ...
       /*  {title: "Cost Price($)", field: "Cost_Price"}, */
        ...
        /*{title: "Category Name", render: rowData => <span>{rowData.category.name}</span>}, 
         {title: "Category", field:"Device_Group"}, */
        {title: "Status",field:"status", render: rowData => { return ((rowData.inventory === rowData.threshold) ? <span style={{backgroundColor:'#f0ad4e',color:'white'}} className="btn">Average</span> : ((rowData.inventory > rowData.threshold) ? <span style={{backgroundColor:'#5cb85c',color:'white'}} className="btn">Good</span> : <span style={{backgroundColor:'#FFBF00',color:'white'}} className="btn">Poor</span>))}},
        {title: "Quantity", field: "inventory"},
        {title: "Actions",render: rowData => <h3><Link to={`/admin/update/inventory/${rowData._id}`}>+/-</Link></h3>},
        {render: rowData => <Link to={`/admin/update/product/${rowData._id}`}><Edit /></Link>},
        {title: "",render: rowData => <span style={{color:"red", cursor:'pointer'}} onClick={() => handleDeleteOpen(rowData._id)}><DeleteOutline /></span>},   
    ]
    ...
    /* const destroy = productId => {
        ...
        {
            setBtnloading(true);
            deleteProduct({id:productId, isSmartDots: false}).then(data => {
                if (data.error) {
                    setBtnloading(false);
                    setError(data.error);
                    setOpen(true);
        ...
        >
            ...
            <DialogContent>
                <FormControl component="fieldset">
                    ...
                    >
                        <FormControlLabel value={1} control={<Radio />} label={"Delete from Master and Webstore Product"} />
                        
                        <FormControlLabel value={2} control={<Radio />} label="Delete from WebStore only." />
                ...
                <div className="col-lg-12 grid-margin stretch-card">
                    <div className="card">
                        <div className="card-body">
                                    ...
                                    <h4 className="card-title" style={{textAlign:'center', color:"#106eea"}}>Manage Webstore</h4>

                                    <MaterialTable
                                    title={<Link to='/admin/add/webstore'>Add webstore product</Link>}
                                    columns={columns}
                                    ...
                                    localization={{ body:{ emptyDataSourceMessage:<h6>No webstore products to display</h6> } }}
                                    />
