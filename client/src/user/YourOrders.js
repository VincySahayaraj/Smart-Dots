...
import {getRestrictUserOrders, updateCancelStatus, createReturnProduct} from './apiUser';
...
const YourOrders = ()=>{
    ...
    const { btnloadingRet,photo,orderid,error1,formData, return_message, return_reason, return_type} = returnVal

    const loadOrders = ()=>{
        getRestrictUserOrders()
        .then(data=>{
            if(data.error){
                setloading(false);
                setOpenCancel(false);
            }else{
                setReturnVal({...returnVal, formData:new FormData()});
                setOrders(data);
                setloading(false); 
                setOpenCancel(false);
            }         
    ...
    const handleChangeforReturn = name => event => {
        const value = name === 'photo' ? event.target.files[0] : event.target.value;
        formData.set(name, value);
        setReturnVal({ ...returnVal, [name]: value });
    }

    const clickSubmitReturn = (event) => {
    
        event.preventDefault();
        setReturnVal({btnloadingRet:true});
        formData.set('orderid', tempOrderid);
        createReturnProduct(formData)
        .then(
            data => {
                if (data.error) {
    ...
    const clickSubmitReturn1 = (ordID) => {
        setTempOrderId(ordID)
        setShowReturn(true);
        return 0;
    }
    ...
    const showReturnModal = () => {
        return (
            ...
            >
                ...
                <Modal.Body>
                <form onSubmit={clickSubmitReturn}>
  
                  <div className="form-group">
                    <label className="text-muted">Reason<span style={{color:"red"}}> *</span></label>
                    <select required
                        onChange={handleChangeforReturn('return_reason')} 
                        className="form-control">
                            ...
                            <option value="Bought the wrong item">Bought the wrong item</option>
                            <option value="Purchase arrived too late">Purchase arrived too late</option>
                    </select>
                </div>
                    <div className="form-group">
                        <label className="text-muted">Message<span style={{color:"red"}}> *</span></label>
                        <input onChange={handleChangeforReturn("return_message")} type="text" className="form-control" value={return_message}  required/>
                    </div>
  
                    <div className="form-group">
                       
                        <input type="hidden" className="form-control" value={orderid} />
                    </div>
  
                    <div className="form-group">
                      <label className="text-muted">Return Type<span style={{color:"red"}}> *</span></label>
                      <select required
                          onChange={handleChangeforReturn('return_type')} 
                          className="form-control">
                   ...
                   <label className="text-muted">Upload Photos</label>
                    ...
                    <div className="form-group">
                        <label className="btn btn-secondary">
                          <input onChange={handleChangeforReturn('photo')} type="file" name="photo" accept="image/*" />
                        </label>
                    </div> 
                    <center>
                        <br/>
                        <button disabled={btnloadingRet} className="btn btn-warning white-color orange-bg font-larger oswald-font shop-btn">{btnloadingRet ? "Loading..." : "Submit"}</button>
                    </center>      
    ...
    const checkDateforRefund = (deliveredDate) => {
        let todayDate = moment().format("YYYY-MM-DD");
        let finaldate = moment(deliveredDate).add(15, 'days').format("YYYY-MM-DD");
        let isRefund = moment(todayDate).isSameOrBefore(finaldate);
        return isRefund
    }
            ...
            {orders.map((item)=>(
                <div key={item._id} className="col-md-12 px-0 mb-2">
                    ...
                    <div className="row">
                        ...
                        <Link to={"view-more/"+item._id} className="btn btn-warning white-color orange-bg font-larger oswald-font shop-btn" style={{borderRadius: '20px'}}>View More</Link>
                        ...
                        {(item.status === "Delivered" && item.shipping) && (checkDateforRefund(item.updatedAt)) && (<button onClick={() => clickSubmitReturn1(item._id)}  className="btn btn-warning white-color orange-bg font-larger oswald-font shop-btn" style={{borderRadius: '25px'}}>Return</button>)}
    ...
    return(
            <UserLayout>
                <div className="user_page_head">
                    <h2 style={{textAlign:'center',color:'#19a6dd'}}>All Orders</h2>
